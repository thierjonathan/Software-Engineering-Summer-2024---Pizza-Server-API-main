...
def update_user(
    ...
    if user_found:
        ...
        logging.error('User could not be found and could not be updated. Id: {}'.format(user_id))
...
def get_user(user_id: uuid.UUID,
    ...
    if not user_found:
        logging.error('User could not be found and could not be retrieved. Id: {}'.format(user_id))
        raise HTTPException(status_code=404, detail=ITEM_NOT_FOUND)
...
def delete_user(
    ...
    if not user_found:
        logging.error('User could not be found and could not be deleted. Id: {}'.format(user_id))
        raise HTTPException(status_code=404, detail=ITEM_NOT_FOUND)
