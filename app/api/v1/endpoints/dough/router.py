...
def create_dough(dough: DoughCreateSchema, request: Request, db: Session = Depends(get_db)):
    ...
    logging.info(f'Creating dough with name: {dough.name}')
    return new_dough
...
def get_dough(dough_id: uuid.UUID,
    ...
    if not dough:
        logging.error('Dough could not be found and could not be retrieved. Id: {}'.format(dough_id))
        raise HTTPException(status_code=404, detail=DOUGH_NOT_FOUND)
...
def delete_dough(dough_id: uuid.UUID, db: Session = Depends(get_db)):
    ...
    if not dough:
        logging.error('Dough could not be found and could not be deleted. Id: {}'.format(dough_id))
        raise HTTPException(status_code=404, detail=DOUGH_NOT_FOUND)
