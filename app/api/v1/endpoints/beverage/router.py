...
def update_beverage(
    ...
    if beverage_found:
        ...
        logging.error('Beverage could not be found and could not be updated. Id: {}'.format(beverage_id))
...
def get_beverage(
    ...
    if not beverage:
        logging.error('Beverage could not be found and could not be retrieved. Id: {}'.format(beverage_id))
        raise HTTPException(status_code=404, detail=BEVERAGE_NOT_FOUND)
...
def delete_beverage(
    ...
    if not beverage:
        logging.error('Beverage could not be found and could not be deleted. Id: {}'.format(beverage_id))
        raise HTTPException(status_code=404, detail=BEVERAGE_NOT_FOUND)
